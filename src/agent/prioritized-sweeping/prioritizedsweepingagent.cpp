...
namespace prioritizedsweeping{
  ...
  PrioritizedSweepingAgent::PrioritizedSweepingAgent(std::unique_ptr<rlinterface::Environment> environment, double gamma, double epsilon, double planning_limit, double priority_threshold):
  environment(std::move(environment)), gamma(gamma), epsilon(epsilon), planning_limit(planning_limit), priority_threshold(priority_threshold){
  ...
  void PrioritizedSweepingAgent::Train(int numberofepisode){
    ...
    while (episodecounter < numberofepisode){
      ...
      while (!currentenvironmentstate->IsTerminal()){
        ...
        // update model
        model.UpdateModel(ExperienceTuple(currentagentstate->GetPureState(),nextaction,nextagentstate->GetPureState(),response->GetReward()));
        LogModel(episodecounter,stepsizecounter);
        // calculate the error
        ...
        while (!priority_queue.empty() && (planning_counter < planning_limit)){
            ...
            LogQtable(episodecounter,stepsizecounter,planning_counter);
  ...
  void PrioritizedSweepingAgent::LogModel(int episode, int step){
      std::string filename = "logs/model/P/model_P_e" + std::to_string(episode) + "_s" + std::to_string(step);
      std::string filename2 = "logs/model/R/model_R_e" + std::to_string(episode) + "_s" + std::to_string(step);
  ...
  void PrioritizedSweepingAgent::LogQtable(int episode, int step, int plan){
      std::string filename = "logs/qtable/qtable_e" + std::to_string(episode) + "_s" + std::to_string(step) + "_p" + std::to_string(plan);
      std::ofstream file(filename);
  ...
  void PrioritizedSweepingAgent::LogExp(int episode, int step){
      std::string filename = "logs/qtable/qtable_e" + std::to_string(episode) + "_s" + std::to_string(step) + "_p" + std::to_string(plan);
      std::ofstream file(filename);
      for(std::list<State>::iterator it1 = qtable.begin(); it1 != qtable.end(); ++it1){
          std::list<Action> actions = it1->GetActionList();
