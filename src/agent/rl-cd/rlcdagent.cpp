    ...
    void RLCDAgent::Train(int numberofepisode){
        ...
        while (episodecounter < numberofepisode){
            ...
            while (!currentenvironmentstate->IsTerminal()){
                ...
                UpdateError err = current_model->UpdateModel(experience_tuple,false);
                // update model qualities and find with maximum quality
                if(log_errors){
                ...
                for(std::list<std::unique_ptr<Model>>::iterator it1 = models.begin(); it1 != models.end(); ++it1){
                    double model_quality = (*it1)->UpdateModelQuality(experience_tuple,Rmax,Rmin);
                    if(model_quality > max){
                        max = model_quality;
                        max_model = (*it1).get();
                    }
                    if(log_errors){
                        errorfile << "("+ std::to_string((*it1)->GetId()) + "," + std::to_string(model_quality) + "),";
                    }
                }
                if(log_errors){
                    errorfile << "]" <<'\n';
