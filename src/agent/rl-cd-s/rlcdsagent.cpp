...
namespace rlcds{
    ...
    RLCDSAgent::RLCDSAgent(std::unique_ptr<rlinterface::Environment> environment, double gamma, double epsilon, double planning_limit, double priority_threshold, std::string log_name,int M,int size,double psudo_zero, double c):
    ...
    void RLCDSAgent::Train(int numberofepisode){
        InitiateLogsFiles();
        ...
        while (episodecounter < numberofepisode){
            ...
            while (!currentenvironmentstate->IsTerminal()){
                ...
                if(log_errors){
                    errorfile << "e" + std::to_string((episodecounter+total_episode_count)) + "_s" +  std::to_string(stepsizecounter) + "[";
                }
                ...
                for(std::list<std::unique_ptr<Model>>::iterator it1 = models.begin(); it1 != models.end(); ++it1){
                    if(current_model->GetId() != (*it1)->GetId()){
                        ...
                        if((current_p == 0) && (other_p == 0)){
                            // lim x/x as x->0 = 1
                            temp_sm = std::max(0.0,(*it1)->GetSM() + log(1));
                            (*it1)->SetSM(temp_sm);
                        }
                        else if((current_p == 0) && (other_p != 0)){
                            // lim a/x as x->0 = inf
                            temp_sm = std::max(0.0, (*it1)->GetSM() + log(other_p / psudo_zero));
                            (*it1)->SetSM(temp_sm);
                        }
                        else{
                            // no math expections
                ...
                for(std::list<NextState>::iterator it = state_action->next_state_list.begin(); it != state_action->next_state_list.end(); ++it){
                    if( it->state_action_nextstate_counter > 0){
                ...
                while (!priority_queue.empty() && (planning_counter < planning_limit)){
                    ...
                    for(std::list<StateAction*>::iterator it = leading_states.begin(); it != leading_states.end(); ++it){
                        ...
                        for(std::list<NextState>::iterator it2 = (*it)->next_state_list.begin(); it2 != (*it)->next_state_list.end(); ++it2){
                            if(it2->state_action_nextstate_counter > 0){
    ...
    int RLCDSAgent::EpsilonGreedyPolicy(std::list<State>::iterator agentstate){
        // O(actions^2)
    ...
    std::list<State>::iterator RLCDSAgent::AddNewStateToQTable(rlinterface::State* state, std::list<State> &qtable){
    ...
    std::list<State>& RLCDSAgent::GetQTable(std::list<State> &qtable){
        return qtable;
    }

    void RLCDSAgent::SetEpsilon(double epsilon){
        this->epsilon = epsilon;
    }
    void RLCDSAgent::SetEnvironment(std::unique_ptr<rlinterface::Environment> environment) {
        this->environment = std::move(environment);
    }

    void RLCDSAgent::LogModel(int episode, int step,Model *model){
        if(log_model){
    ...
    void RLCDSAgent::LogQtable(int episode, int step, int plan,Model *model){
        if(log_qtable){
    ...
    void RLCDSAgent::LogExperience(int episode, int step, ExperienceTuple &tuple){
        if(log_experience){
          if(step == 0){
              expfile.close();
    ...
    void RLCDSAgent::LogHistory(int episode, int step){
        if(log_history){
            historyfile << "episode : " << episode << "  stepsize : " << step <<'\n';
        }
    }
    void RLCDSAgent::InitiateLogsFiles(){
    ...
    void RLCDSAgent::CloseLogsFiles(){
        if(log_history){
            historyfile.close();
            changefile.close();
        }
    ...
    void RLCDSAgent::SetLogModel(bool x){
        log_model = x;
    }
    void RLCDSAgent::SetLogQtable(bool x){
        log_qtable = x;
    }
    void RLCDSAgent::SetLogExperience(bool x){
        log_experience = x;
    }
    void RLCDSAgent::SetLogHistory(bool x){
        log_history = x;
    }
    void RLCDSAgent::SetLogErrors(bool x){
        log_errors = x;
    }
}
