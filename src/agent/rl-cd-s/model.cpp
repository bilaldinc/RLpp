...
namespace rlcds{
    ...
    double Model::GetTrasitionEstimate(rlinterface::State *state,int action, rlinterface::State *next_state){
        ...
        bool state_action_is_found = false;
        bool state_is_found = false;

        std::list<StateM>::iterator it0;
        for(it0 = state_list.begin(); it0 != state_list.end(); ++it0){
            if(*it0->state == *state){
                // state is found
                state_is_found = true;
                break;
        ...
        if(state_is_found){
            std::list<StateAction>::iterator it1;
            for(it1 = it0->action_list.begin(); it1 != it0->action_list.end(); ++it1){
                if(it1->action == action){
                    // state action is found
                    state_action_is_found = true;
                    break;
                }
            }
            if(state_action_is_found){
                std::list<NextState>::iterator it2;
                for(it2 = it1->next_state_list.begin(); it2 != it1->next_state_list.end(); ++it2){
                    if(*it2->state == *next_state){
                        // next state is found
                ...
                if(!next_state_is_found){
                    /* next state not is found. It means in this state action,
                    another next state had observed before and this state's
                    probability is updateted implicitly with
                    P(s,a,k) + (1/Nsa + 1) * (0 - P(s,a,k))
                    which is zero because Nsa is zero at the begginng
                    */
                    return 0.0;
                }
            }
            else{
                // never visited or updated before return default
                return 1.0 / size;
            }
        }
        else{
    ...
    std::list<StateAction*> Model::GetStatesLeadsToS(rlinterface::State *state){
        ...
        for(std::list<StateM>::iterator it0 = state_list.begin(); it0 != state_list.end(); ++it0){
            for(std::list<StateAction>::iterator it1 = it0->action_list.begin(); it1 != it0->action_list.end(); ++it1){
                for(std::list<NextState>::iterator it2 = it1->next_state_list.begin(); it2 != it1->next_state_list.end(); ++it2){
                    if((*it2->state == *state) && (it2->state_action_nextstate_counter > 0)){
